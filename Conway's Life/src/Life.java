/*
 * Program is an implementation for Conway's life simulation
 * 
...
public class Life implements MouseListener, ActionListener, Runnable{
	...
	public void mouseReleased(MouseEvent event) {
		...
		@Override
		public void actionPerformed(ActionEvent event) { //Action events for step, start and stop
		...
		public void step() {
			...
			for (int row = 0; row < cells.length; row++) {
				for (int column = 0; column < cells[0].length; column++) {
					...
					if (row > 0 && cells[row-1][column] == true) {//up middle
						neighborCount++;//done
					} 
					if (row > 0 && column < cells[0].length-1 && cells[row-1][column+1] == true) {//up right
						neighborCount++;//done
					} 
					if (column > 0 && cells[row][column-1] == true) {//middle left
						neighborCount++;//done
					} 
					if (column < cells[0].length-1 && cells[row][column+1] == true) {//middle right
						neighborCount++;//done
					} 
					if (row < cells.length-1 && column > 0 && cells[row+1][column-1] == true) {//down left
						neighborCount++;//done
					} 
					if (row < cells.length-1 && cells[row+1][column] == true) {//down middle 
						neighborCount++;//done
					} 
